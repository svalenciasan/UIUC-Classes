...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    //If empty could go after for loop?
    // if (elements.size() == 0) {
    //     return 0;
    // }

    for (size_t i = 0; i < elements.size(); ++i) {
        //If val is < elements[0] return 0(only possible in index 0), If val is == elements[0] or any value in elements, return current index.
