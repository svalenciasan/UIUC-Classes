...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
     ...
     */
    //Right
    //mid_elem_itr + 1 to avoid the mid element that was moved up
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    // if (subroot->is_leaf && (first_larger_idx >=  subroot->elements.size() || !(subroot->elements[first_larger_indx] == pair) )) {
    //     subroot->elements.insert(subroot.begin() + first_larger_idx, pair);
    // }
    bool exist = first_larger_idx <  subroot->elements.size() && subroot->elements[first_larger_idx] == pair;
    if (subroot->is_leaf) {
        if (exist) { 
            return;
        } else {
        ...
        if (exist) { 
            ...
            //Recurse back. Check if child is balanced.
            //order - 1 because elements is order -1 or the number of children they can have
