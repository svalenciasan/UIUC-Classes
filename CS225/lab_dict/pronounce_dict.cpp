...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    for (auto & char1: first) {
        char1 = toupper(char1);
    }
    for (auto & char2: second) {
        char2 = toupper(char2);
    }

    //Check if words exist
    auto firstDict = dict.find(first);
    auto secondDict = dict.find(second);
    if (firstDict == dict.end() || secondDict == dict.end()) {
        return false;
    }

    //Check if same number of pronunciations
    if (firstDict->second.size() != secondDict->second.size()) {
        return false;
    }

    for (size_t i = 0; i < firstDict->second.size(); ++i) {
        if (firstDict->second[i] != secondDict->second[i]) {
            return false;
        }
    }
