...
class KDTree
{
  ...
  private:
    ...
    Point<Dim> findNearestNeighborHelper(KDTreeNode* currNode, const Point<Dim>& query, int dimension) const;
    /**Clears tree**/
    void clearTree(KDTreeNode* currRoot);
    /**Creates deep copy of an existing tree.**/
    KDTreeNode* copyTree(KDTreeNode* currRoot);
    /**Builds tree from list.**/
    KDTreeNode* buildTree(vector<Point<Dim>>& list, int left, int right, int dimension);
    /** Partitions list according to the pivotIndex. Left of pivot index is less, right is more. **/
    void select(vector<Point<Dim>>& list, int left, int right, int k, int dimension);
    /** Partitions list according to the pivotIndex. Left of pivot index is less, right is more. **/
    int partition(vector<Point<Dim>>& list, int left, int right, int pivotIndex, int dimension);
    /** Swaps index first and second in the list **/
    void swap(vector<Point<Dim>>& list, int first, int second);
};
