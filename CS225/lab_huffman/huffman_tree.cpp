...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (singleQueue.size() + mergeQueue.size() != 1) {
        TreeNode* removed_1 = removeSmallest(singleQueue, mergeQueue);
        TreeNode* removed_2 = removeSmallest(singleQueue, mergeQueue);
        TreeNode* parent = new TreeNode(removed_1->freq.getFrequency() + removed_2->freq.getFrequency());
        parent->left = removed_1;
        parent->right = removed_2;
    ...
    while (bfile.hasBits()) {
        ...
        if (bfile.getNextBit() == 0) {
            if (current->left == nullptr) {
                ...
                current = root_;
            } else if (!bfile.hasBits()) {
                ss << current->left->freq.getCharacter();
            }
            ...
            if (current->right == nullptr) {
                ...
                ss << current->right->freq.getCharacter();
            }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.getNextBit() == 1) {
        TreeNode* new_node = new TreeNode(Frequency(bfile.getNextByte(),0));
        return new_node;
    } else {
